...
def test_form_groups_with_minimum_persons_and_maximum_range():
    ...
    groups = form_groups(2, 4, test_users_to_interests, test_interests_to_users, set())
...
def form_groups_with_less_popular_interests():
    random.seed(3123123123)       
    groups = form_groups(2, 4, test_users_to_less_popular_interests, test_interests_to_users, set())
    for group in sorted(groups, key=lambda g: (g.interest, g.users)):
...
def test_form_groups_with_integer_interests():       
    try:
        form_groups(2, 4, test_users_to_interests_integers, test_interests_to_users, set()) 
